...
export class SurgeUpdate {
   ...
   *
   * Metrics calculated:
   * - exchangeToOracleUpdate: Time from most recent exchange price (source_ts_ms) to oracle signing
   * - oracleUpdateToClient: Time from oracle signing to client receipt
   * - endToEnd: Total time from exchange price to client receipt
   *
   * Note: source_ts_ms represents the most recent exchange timestamp across all feeds in the bundle.
   * For per-feed timestamp accuracy, use getFeedTimestamps() instead.
   *
   * For heartbeat updates (triggered_on_price_change=false), the exchangeToOracleUpdate
