 ...
 */
function validateOracleJob(data: unknown): {
  success: boolean;
  data?: unknown;
  error?: string;
} {
  ...
  if (!NonEmptyArrayUtils.safeValidate(obj.tasks as unknown[])) {
    return {
      success: false,
...
export function normalizeOracleJob(
  ...
  const parseJobObject = (data: object) => {
    if (!data) throw new OracleJobError(`No job data provided: ${data}`);
    const validation = validateOracleJob(data);
    if (!validation.success) throw new OracleJobError(validation.error!);
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    return OracleJob.fromObject(validation.data as any);
  };
